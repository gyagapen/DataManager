...
public class AppLauncher extends SherlockFragmentActivity {
	...
	// SharedPreferences access mode
	public static int SHARED_ACCESS_MODE = populateSharedPrefsAccordingToApi();
	...
	private void setUpSideBar(Bundle savedInstanceState) {
		...
		mMenuAdapter = new MenuListAdapter(AppLauncher.this, title, icon);
	...
	private void applySettings() {
		...
		boolean isServiceDeactived = sharedPrefsEditor
				.isServiceDeactivatedAll();
		boolean isServiceDeactivatedPlugged = sharedPrefsEditor
		...
		if (isServiceDeactived
			...
			StartDataManagerService(this, sharedPrefsEditor, logsProvider);
		}
	 ...
	 */
	public static void stopDataManagerService(Context aContext,
			SharedPrefsEditor sharedPrefsEditor, LogsProvider logsProvider) {

		logsProvider.info("service will be stopped");

		// register service stopped in preferences
		sharedPrefsEditor.setServiceActivation(false);
		aContext.stopService(new Intent(aContext.getApplicationContext(),
				MainService.class));

	}

	public static void StartDataManagerService(Context aContext,
			SharedPrefsEditor sharedPrefsEditor, LogsProvider logsProvider) {

		logsProvider.info("service will be started");
		sharedPrefsEditor.setServiceActivation(true);
		Intent serviceIntent = new Intent(aContext.getApplicationContext(),
				MainService.class);
	...
	public static int populateSharedPrefsAccordingToApi() {
		int accessMode = 0;

		int currentapiVersion = android.os.Build.VERSION.SDK_INT;
		if (currentapiVersion <= android.os.Build.VERSION_CODES.GINGERBREAD) {
			accessMode = Activity.MODE_PRIVATE;
		} else {
			accessMode = Activity.MODE_MULTI_PROCESS;
		}
